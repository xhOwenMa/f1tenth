...
class LaneNetImageProcessor():
    def __init__(self, weights_path, image_width, image_height, max_lane_y=420, WARP_RADIUS=20, WP_TO_M_Coeff=[1,1], lane_sep=50, new_lane_x=20, new_lane_y=100, max_y_gap=25, min_lane_pts=10):
        ...
        dst = np.float32([[image_width/2 - WARP_RADIUS, 0], [image_width/2 + WARP_RADIUS, 0], [0, max_lane_y - 1], [image_width - 1, max_lane_y - 1]])
    ...
    def init_lanenet(self):
        ...
        self.postprocessor = lanenet_postprocess.LaneNetPostProcessor(self.image_width, self.image_height, 512, 256, self.max_lane_y, self.WARP_RADIUS)
    ...
    def image_to_trajectory(self, cv_image, x_pos, y_pos, yaw, lane_fit=True):
        ...
        warped = cv2.warpPerspective(cv_image[self.image_height - self.max_lane_y : self.image_height], self.M, (self.image_width, self.max_lane_y))
        warped2 = warped.copy()
        ...
        for i in range(len(lanes)):
            ...
            for i in range(20):
                cv2.circle(warped2, (np.int_(dx[i]), np.int_(dy[i])), 5, (0, 255, 0), -1)
	...
	"""
        ...
        if self.full_lane_pts and self.full_lane_pts[0][0][0] < self.image_width/2 and self.full_lane_pts[-1][0][0] > self.image_width/2:
        ...
        if changed:
            ...
            if self.right_lane is not None:
                ...
                self.right_lane = Polynomial.fit(self.right_lane_pts[1], self.right_lane_pts[0], self.right_lane.degree())
    ...
    def compare_lanes(self, old, shifted_p, num_pts=20, max_avg_distance=20):
        ...
        return np.sum(np.abs(old(dy) - shifted_p(dy))) / num_pts <= max_avg_distance and shifted_p.domain[1] >= old.domain[1]
