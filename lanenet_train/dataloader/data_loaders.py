...
class TusimpleSet(Dataset):
    def __init__(self, dataset, n_labels=3, transform=None):
        self._gt_img_list = []
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            transformed = self.transform(image=img, masks=[label_img, label_instance_img])
            img = transformed["image"]
            label_img = transformed["masks"][0]
            label_instance_img = transformed["masks"][1]
            

        label_binary = np.zeros([label_img.shape[0], label_img.shape[1]], dtype=np.uint8)
        mask = np.where((np.array(label_img)[:, :, :] != [0, 0, 0]).all(axis=2))
